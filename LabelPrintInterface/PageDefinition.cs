...
namespace LabelPrintInterface
{
    public class PageDefinition
    {
        ...
        protected int totalLabelsPerSheet;
        //public string resizeType;
        //  public GridLayout grid;
        public List<Point> lstPoint = new List<Point>();
        protected GridLayout gridLayout;

        public GridLayout Grid {get{return gridLayout;} }
        public PageDefinition(int marginleft, int margintop, int availablelabelwidth, int availablelabelheight, int xgapbetweenlabels, int ygapbetweenlabels, int colsperrow, int totalrows, int clockwisedegreeshift, string resizemode)
        {
            ...
            gridLayout = new GridLayout(marginleft, margintop, availablelabelwidth, availablelabelheight, xgapbetweenlabels, ygapbetweenlabels, colsperrow, totalrows, clockwisedegreeshift, resizemode);
            //do all the offset calculation up front...
            lstPoint = gridLayout.lstPoint;

        }
        public PageDefinition(int totalLabels)
        {
            totalLabelsPerSheet = totalLabels;
            for (int i = 0; i < totalLabelsPerSheet; i++)
            {
                lstPoint.Add(new Point(0, 0));
            }
            //clockwiseDegreeShift = 0;
            //resizeType = "none";
        }
        ...
        //                lb.templatewidth = gridLayout.availableLabelWidth;
        //                lb.templateheight = gridLayout.availableLabelHeight;
        //                if (lb.width == null) { lb.width = gridLayout.availableLabelWidth; }
        //                if (lb.height == null) { lb.height = gridLayout.availableLabelHeight; }
        //            }
        //            foreach (TextItem item in lb.lstTxt)
        //            {
        //                item.positionX += lstPoint[pageIndex].X;
        //                item.positionY += lstPoint[pageIndex].Y;
        //            }
